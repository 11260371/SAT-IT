...
parser.add_argument('--model', default='./checkpoints/rslad_tinyimagenet',
...
log_dir = './logs/rslad_tinyimagenet'
...
def train(args, model, device, train_loader, optimizer, epoch, teacher_model):
    model.train()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = rslad_inner_loss(model=model,
...
def main():
    ...
    teacher_model = ResNet18_tinyimagenet200().to(device)
    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)

    # Replace with your pretrained teacher checkpoint (.pt/.pth)
    teacher_model_path = "./checkpoints/teacher_mart.pt"
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train(args, model, device, train_loader, optimizer, epoch, teacher_model)
