...
from model.resnet import ResNet18,ResNet18_100
from model.resnet_tinyimagenet import *
from ema_mart_weight_reg3_2 import ema_loss
import numpy as np
import time
...
parser.add_argument('--model', default='./checkpoints/sat_it',
...
trainset_indexed = IndexedDataset(trainset)

train_loader = DataLoader(trainset_indexed, batch_size=args.batch_size, shuffle=True, num_workers=10)


test_loader = DataLoader(testset, batch_size=args.test_batch_size, shuffle=False, num_workers=10)
...
def train(args, model, device, train_loader, optimizer, epoch, ema_probs):
    model.train()
...
def main():
    ...
    ema_probs = torch.zeros((len(trainset), 10), device=device)
    epochs_list = []
    natural_acc = []
    robust_acc = []
    loss_list = []
    ...
    # Set > 0 to resume from model_dir checkpoints at this epoch (model/opt/ema .pt files).
    # Set > 0 to resume from model_dir checkpoints at this epoch (model/opt/ema .pt files).
    checkpoint_epoch = 0

    if os.path.exists(file_name) and checkpoint_epoch > 0:
        data = np.load(file_name)
        epochs_list = list(data['epochs'])[:checkpoint_epoch]
        natural_acc = list(data['natural_acc'])[:checkpoint_epoch]
        robust_acc = list(data['robust_acc'])[:checkpoint_epoch]
        loss_list = list(data['loss'])[:checkpoint_epoch]

    if checkpoint_epoch and checkpoint_epoch > 0:
        # Resume files under model_dir — replace epoch index or point to your .pt/.pth files
        ...
        ema_path = os.path.join(model_dir, f'ema{checkpoint_epoch}.pt')

        if os.path.exists(model_path) and os.path.exists(opt_path) and os.path.exists(ema_path):
            print(f"Loaded checkpoint; resume training from epoch {checkpoint_epoch + 1}")
            model.load_state_dict(torch.load(model_path, map_location=device))
            optimizer.load_state_dict(torch.load(opt_path, map_location=device))
            ema_probs = torch.load(ema_path, map_location=device)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        epoch_loss = train(args, model, device, train_loader, optimizer, epoch, ema_probs)
        ...
        if epoch % args.save_freq == 0 and epoch >=75:
            ...
            torch.save(ema_probs, os.path.join(model_dir, f'ema{epoch}.pt'))
