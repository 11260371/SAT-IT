...
def extract_model_state_dict(obj):
    """Extract a model state dict from a torch.load() result."""
...
def attack_pgd(model, batch_data, batch_labels, attack_iters=20, step_size=0.003, epsilon=8.0 / 255.0):
    ce_loss = nn.CrossEntropyLoss().to(batch_data.device)
...
def eval_pgd_blackbox(student, teacher, testloader, attack_iters=20, step_size=0.003, epsilon=8.0 / 255.0):
    print("=============== PGD Black-box Transfer Attack Evaluation ===============")
    ...
    print(f"[RESULT] student robust acc under PGD black-box transfer attack: {robust_acc:.4f}")
